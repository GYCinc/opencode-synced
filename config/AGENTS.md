...
#### ASK or READ or SEARCH > GUESS or PATTERN MATCH ALWAYS! That means: use the Call question tool to ask the user questions befor during and after execution when needed.


# **NO NPM/NPX .** This is a npm, npx, node (any Node-based tooling unless there is 0 choice) FREE PROJECT AND COMPUTER!.

# Always and ONLY Use Bun, cargo, or any language-native package manager. If a tool exists as a uv tool or bunx alternative, use that. Always. No exceptions. No "it's just a devDependency." No.  
...
- Always search for available skills before performing any task of consequence or complexity that takes more than one step. If there is not a skill available, search. A skill appropriate to the aforementioned event is often the shortest route to task success and completion. Consider using the skill chain 
...
### ***SeqThink MCP*** - At times unique and novel problems and situations arise. In such cases you are a prolific sequential thinker and value the iterative process of objective self reflection. Before answering complex problems, show your reasoning in numbered steps. Revise openly when wrong. Branch when alternatives matter. Conclude clearly. Self-verify before any git push or significant addition to the memory.


### Strategies — pick one based on the problem:

Linear: default. flexible step-by-step. pause, research, revise. use when unsure.
Chain of Thought: straightforward logic. straight line A to B to C.
ReAct: think, act (use a tool), observe, repeat. for when you need external info.
ReWOO: plan all lookups upfront, execute in parallel, then solve.
Scratchpad: track variables and iterate. for math and algorithms.
Self-Ask: break into sub-questions, answer each, combine.
Self-Consistency: solve 3+ ways, go with the majority.
Step-Back: find the general principle first, then apply to specifics.
Tree of Thoughts: develop 2-3 approaches in parallel, evaluate, pick best.
...
#### GITHUB ACCOUNTS:  
- *** GYCinc (everything-else). Switch: gh auth switch.  
- *** preplyaironislive (student records). Same switch command to toggle.  
- Both stored in gh keyring. No tokens to juggle.  


## REMEBER! 

**Skills-first:** Always check for skills before building. openskills list to see available. openskills read  to load.

**Success Protocols:** Read ~/.config/opencode/SUCCESS-PROTOCOLS.md before every session.
